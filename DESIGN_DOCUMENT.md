...
| D12 | Two test layers: the client offline with `aioresponses` (no Home Assistant needed) and the integration under `pytest-homeassistant-custom-component` with a scripted `FakeClient` and an in-memory recorder. Coverage is held at ≥ 95 % in CI; every quality-scale rule is tracked in `quality_scale.yaml`, but no tier is declared because the bundled client leaves `dependency-transparency` (Bronze) and `async-dependency` (Platinum) open. | The client is where the protocol risk is; the HA layer is where the statistics arithmetic and flow wiring can silently go wrong (the tests found an off-by-one bucket in the running-total lookup). |
...
| 9 | ID discovery, mirroring the portal's own start-up calls: `apex://cm_AccountBillingUsageAURA/ACTION$getBillingAccountsForUser` (no params) → first `Billing_Account__c` with `Status__c = Active` (else the first); then `…$getMetersByPropertyIds` with `propertyIds=[<Property__c>]` → first meter with `Is_Digital__c` or `Digital_Meter__c` true (`Name` is the serial). Both return values are JSON **strings** that must be decoded. Until 2026-09 this used `retrieveBillingAccounts` / `retrieveSObject` with a field list and `whereClause`; the portal now drops those actions from the response without an error. | `baId`/`meterId` are Salesforce record ids (`a08…`, `a1K…`), not the account number or meter serial. The meter is linked to the *property*, not the billing account. Captured live 2026-09-15; discovery re-captured 2026-09-24. |
...
| First scheduled 02:00 poll on a real Home Assistant. | **Done 2026-09-18** on v2.0.0b5: the poll ran at 02:00 local and imported a new day of readings on the author's production instance. Released as v2.0.0. |
